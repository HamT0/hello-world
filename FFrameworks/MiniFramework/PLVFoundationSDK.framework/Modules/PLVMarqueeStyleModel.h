...
typedef NS_ENUM(NSUInteger, PLVMarqueeViewType) {
    /// 滚动（自屏幕右方至左方一直滚动）
    PLVMarqueeViewTypeRoll = 1,
    /// 闪烁（屏幕内随机位置闪烁）
    PLVMarqueeViewTypeFlash,
    /// 滚动+闪烁（自屏幕右方至左方一直滚动，渐隐渐现）
    PLVMarqueeViewTypeRollFade,
    /// 局部滚动（上下15%的视频区域之间滚动）
    PLVMarqueeViewTypePartRoll,
    /// 局部闪烁（上下15%的视频区域随机闪烁文字）
    PLVMarqueeViewTypePartFlash,
    /// 滚动（自屏幕右方至左方一直滚动）双跑马灯
    PLVMarqueeViewTypeDoubleRoll,
    /// 闪烁（屏幕内随机位置闪烁））双跑马灯
    PLVMarqueeViewTypeDoubleFlash,
};

/// 负责定义跑马灯2.0样式和动画
@interface PLVMarqueeStyleModel : NSObject
/// 跑马灯样式
@property (nonatomic, assign) PLVMarqueeViewType style;
...
+ (instancetype)createMarqueeModelWithContent:(NSString *)content
                                    ...
                                    fontColor:(NSString *)fontColor
                                        alpha:(float)alpha
                                        style:(PLVMarqueeViewType)style;
